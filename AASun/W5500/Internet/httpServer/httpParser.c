	...
	)
{
	...
	snprintf(tmp, sizeof (tmp), "%ld", len);
	strcpy(buf, head);
	strcat(buf, tmp);
	strcat(buf, "\r\n\r\n");
}
	...
	)
{
  ...
  char * nexttok;
  nexttok = strtok_r((char*)buf," ", & pSave);
  if(!nexttok)
  {
  ...
  if(!strcmp(nexttok, "GET") || !strcmp(nexttok,"get"))
  {
    ...
    nexttok = strtok_r(NULL," ", & pSave);

  }
  else if (!strcmp(nexttok, "POST") || !strcmp(nexttok,"post"))
  {
    nexttok = strtok_r(NULL,"\0", & pSave);
...
uint8_t get_http_uri_name(uint8_t * uri, uint8_t * uri_buf)
{
	...
	// Copy the string to not modify the original string
...
// AdAstra rewrite to use a much smaller uri_buf, doesn't use strtok()
uint8_t get_http_uri_name(uint8_t * uri, uint8_t * uri_buf)
{
	char		* uri_ptr ;
	uint32_t	len ;
...
void inet_addr_(uint8_t * addr, uint8_t *ip)
{
	...
	for(i = 0; i < 4 ; i++)
	{
		nexttok = (uint8_t *)strtok_r((char *)nexttok, ".", & pSave);
