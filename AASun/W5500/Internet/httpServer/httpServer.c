...
#ifdef _USE_FLASH_
mfsCtx_t	wMfsCtx ;	// MFS file system context
#endif

/*****************************************************************************
 * Private functions
 ****************************************************************************/
void httpServer_Sockinit(uint8_t cnt, const uint8_t * socklist);
...
void httpServer_init(uint8_t * tx_buf, uint8_t * rx_buf, uint8_t cnt, const uint8_t * socklist)
{
...
//					if ((len = getSn_RX_RSR(s)) > 1)
					if ((len = getSn_RX_RSR(s)) > 0)
					{
						...
						// AdAstra: check if recv return value is an error number
						if ((int16_t)len < 0)
						{
							aaPrintf ("recv error %d\n", (int32_t)(int16_t)len) ;
							return ;
						}
...
//#endif
		...
		case SOCK_CLOSED:
			...
			// https://github.com/Wiznet/ioLibrary_Driver/issues/80
			HTTPSock_Status[seqnum].file_len    = 0;
			HTTPSock_Status[seqnum].file_offset = 0;
			HTTPSock_Status[seqnum].file_start  = 0;
			HTTPSock_Status[seqnum].sock_status = STATE_HTTP_IDLE;
...
static void send_http_response_body(uint8_t s, uint8_t * uri_name, uint8_t * buf, uint32_t start_addr, uint32_t file_len)
{
	...
	if(HTTPSock_Status[get_seqnum].file_len == 0) // ### Send HTTP response body: First part ###
	{
...
/*****************************************************/
...
#ifdef _USE_SDCARD_
	if(HTTPSock_Status[get_seqnum].storage_type == SDCARD)
	{
	...
	{
		// Data read from external data flash memory: read_from_flashbuf(addr, &buf[0], send_len);
		uint32_t offset = HTTPSock_Status[get_seqnum].file_start + HTTPSock_Status[get_seqnum].file_offset;
		mfsSeek	(& HTTPSock_Status[get_seqnum].mfsFile, offset, MFS_SEEK_SET) ;
		mfsRead (& HTTPSock_Status[get_seqnum].mfsFile, buf, send_len) ;
		*(buf+send_len+1) = 0; // Insert '/0' for indicates the 'End of String' (null terminated)
	}
	else
#endif
	...
	if(send_len)
	{
		// AdAstra: beware the socket may be busy and then returns a len value of 0
		int32_t len = send(s, buf, send_len);
		if (len > 0)
		{
			HTTPSock_Status[get_seqnum].file_offset += len;
#ifdef _HTTPSERVER_DEBUG_
...
//			aaPuts ("SOCK_BUSY\n") ;
	...
	if (HTTPSock_Status[get_seqnum].file_offset == HTTPSock_Status[get_seqnum].file_len)
	{
...
// ## 20141219 Eric added, for 'File object structure' (fs) allocation reduced (8 -> 1)
#ifdef _USE_FLASH_
		mfsClose (& HTTPSock_Status[get_seqnum].mfsFile) ;
#endif
#ifdef _USE_SDCARD_
		f_close(&fs);
#endif
...
#endif
	// AdAstra: this allows only to  send string!!!
//	send_len = snprintf((char *)buf, DATA_BUF_SIZE, "%s%d\r\n\r\n%s", RES_CGIHEAD_OK, file_len, http_body);

	// This allows to send binary data
	send_len = snprintf((char *)buf, DATA_BUF_SIZE, "%s%d\r\n\r\n", RES_CGIHEAD_OK, file_len);
	memcpy (buf+send_len, http_body, file_len) ;
	send_len += file_len ;
...
//  Very big array on stack !!!!!!
//	uint8_t uri_buf[MAX_URI_SIZE]={0x00, };
	uint8_t uri_buf[MAX_URI_NAME]={0x00, };
	...
	switch (p_http_request->METHOD)
	{
      ...
      printf("HTTPSocket[%d] : http_process_handler():: METHOD_GET\r\n", s);
			...
			{
				uint32_t lenMax = DATA_BUF_SIZE - (strlen (RES_CGIHEAD_OK) + 8) ;
				content_found = http_get_cgi_handler(uri_name, pHTTP_TX, lenMax, & file_len);
				if(content_found && (file_len <= lenMax))
				{
...
#endif
#ifdef _USE_FLASH_
//				if(/* Read content from Dataflash */)
				// mfs need an absolute path for files, so in the web pages path always begins with ./
				if (MFS_ENONE == mfsOpen (& wMfsCtx, & HTTPSock_Status[get_seqnum].mfsFile,
						(const char *) ((uri_name [0] == '.') ? (uri_name + 1) : uri_name)))
				{
					content_found = 1;
					file_len = mfsSize (& HTTPSock_Status[get_seqnum].mfsFile) ;
					content_addr = 0;
					HTTPSock_Status[get_seqnum].storage_type = DATAFLASH;
#ifdef _HTTPSERVER_DEBUG_
					printf("> HTTPSocket[%d] : mfsOpen OK: %s\r\n", s, (uri_name [0] == '.') ? (uri_name + 1) : uri_name);
#endif
				}
#endif
				else
				{
#ifdef _HTTPSERVER_DEBUG_
					printf("> HTTPSocket[%d] : mfsOpen error: %s\r\n", s, (uri_name [0] == '.') ? (uri_name + 1) : uri_name);
#endif
