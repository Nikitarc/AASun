...
#include	"aa.h"		// For BSP_DEBUG_BKPT
...
//	}
	...
	// On reset the system clock is 16 MHz from HSI
	// The divide by 8 of the SYSTICK is set, so the SYSTICK clock is 2 MHz
	// For a timeout of 10ms the tick count is 20000
	// When the COUNTFLAG is set the timeout has elapsed
	SysTick->LOAD  = 20000u - 1u ;						// Set reload register
	SysTick->VAL   = 20000u - 1u ;						// Load the SysTick Counter Value
	SysTick->CTRL  =
//						SysTick_CTRL_TICKINT_Msk   |	// Enable interrupt
//						SysTick_CTRL_CLKSOURCE_Msk |	// Divider 8 if 0
						SysTick_CTRL_ENABLE_Msk ;		// Enable SysTick Timer
	while ((SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk) == 0  &&  LL_RCC_HSE_IsReady() == 0)
	{
	}
	...
	{
		// HSE not ready, use HSI (started on reset)
		LL_RCC_HSE_Disable();
...
*/
// ----------------------------------------------------------------------------
// Use external 10MHz VCTCXO as clock source, system clock is 64 MHz
// Don't forget to set HSE_VALUE=10000000 in the tool settings
/*
void SystemClock_Config(void)
{
	LL_FLASH_SetLatency(LL_FLASH_LATENCY_2);
	...
	// Main PLL configuration and activation
	LL_RCC_PLL_ConfigDomain_SYS(LL_RCC_PLLSOURCE_HSE, LL_RCC_PLLM_DIV_1, 32, LL_RCC_PLLR_DIV_5);
	LL_RCC_PLL_Enable();
	LL_RCC_PLL_EnableDomain_SYS();
	while (LL_RCC_PLL_IsReady() != 1)
	{
...
// ----------------------------------------------------------------------------
//	This function is used by CubeMX generated code

BSP_ATTR_USED static	void   Error_Handler	(void)
{
	BSP_DEBUG_BKPT () ;
	while (1)
	{
